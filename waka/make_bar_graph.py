...
class BarGraph:
    ...
    def build_graph(self):
        ...
        for year in self.yearly_data.keys():
            for quarter in self.yearly_data[year].keys():
                for language in sorted(
                    list(self.yearly_data[year][quarter].keys()),
                    key=lambda lang: self.yearly_data[year][quarter][lang],
                    reverse=True,
                )[0:max_languages]:
        ...
        for language in languages_all_loc.keys():
            language_df[language] = pd.DataFrame(
                languages_all_loc[language], index=list(self.yearly_data.keys()), columns=["Q1", "Q2", "Q3", "Q4"]
            )
        ...
        chart = (
            ...
            .encode(
                ...
                y=alt.Y("sum(values):Q", axis=alt.Axis(grid=False, title="LOC added")),
